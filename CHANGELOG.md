...
- 63dd27f: CI now runs `check:barrel` alongside `check:tokens` (so missing component CSS imports cannot ship). Added Button tests locking the #56 loading accessible-name fix. README Install/Setup corrected: publish via GitHub Packages, and `@eq-solutions/tokens` is a peerDependency (not auto-installed).

## 1.16.7

### Patch Changes

- 1217a56: AppShell: bump inactive sidebar nav contrast (label 0.45→0.62, icon-chip 0.06→0.10) so icons/labels actually read on the dark sidebar; same bump for collapse/logout/density chrome. Fix muted nav counts (`--eq-slate` was never a token — they stayed sky). Add `:focus-visible` to sidebar nav items and a Modal-style focus trap + focus restore on the mobile drawer.
